...
public class NetworkUtils {
	 ...
	 * @param ipAddress use for get name of this ip
	 * @return computer name
	 */
	public static String getHostNameByCommandLine(String ipAddress) {
		log.debug("Enter getHostNameByCommandLine");
		String hostName = null;
		try {
//			remoteHostName = InetAddress.getByName(ipAddress).getCanonicalHostName();
			...
			command.append(" -n 1 ");		// -n count Number of echo requests to send.
			command.append("\"");
			...
			//substring to get hostname Ex. Pinging NATTAPHONG-T [157.179.132.232] with..........
...
//			command.append("cmd /C ");
//			command.append("\"");
//			command.append(" nbtstat -a ");	// -A   (Adapter status) Lists the remote machine's name table given its IP address.
//			command.append(ipAddress);
//			command.append("\"");
		} catch (IOException e) {
	...
	/**
	 * this method managing related session attribute {@link Constants#SESSION_REMOTE_NETWORK_DETAIL}<BR/>
	 * then get detail of remoting client in {@link NetworkDetail} pojo <BR/>
	 * 
	 * @return {@link NetworkDetail} pojo 
	 */
	public static NetworkDetail managingSessionNetworkDetail() {
